...
const SalesLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.role === 'admin') {
        setError('This account is not a Sales Team member. Use the Admin login instead.');
      } else {
        router.push('/sales/dashboard');
      }
    } catch (err) {
      const msg = err.response?.data?.message;
      setError(msg || (err.request ? 'Could not reach the server. Please try again.' : 'Invalid email or password'));
    }
  };
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md px-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 md:p-10">
          <div className="text-center mb-8">
            ...
            <h1 className="font-display text-3xl mb-2">Sales Team Sign In</h1>
            <p className="text-gray-400 text-sm">Restricted area — Sales Team only.</p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative"><FaEnvelope className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" /><input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} required className="input-field !pl-12 rounded-2xl" placeholder="sales@solscape.com" /></div>
            </div>
            <div>
            ...
            <button type="submit" className="btn-primary w-full text-[10px]"><span>Sign In to Sales Dashboard</span></button>
          </form>
          <p className="text-center text-gray-400 text-sm mt-8">Not a Sales Team member? <Link href="/" className="text-luxury-accent hover:underline font-medium">Back to website</Link></p>
        </div>
      </motion.div>
    </section>
  );
