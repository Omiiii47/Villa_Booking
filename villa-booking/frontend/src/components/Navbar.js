            ...
            <div className="hidden lg:flex items-center gap-1">
              ...
              <div className="ml-6 flex items-center gap-3">
                {user ? (
                  <>
                    {user.role === 'admin' && (
                      <Link to="/admin" className={`font-body text-xs uppercase tracking-[0.2em] font-medium transition-colors ${
                        scrolled ? 'text-luxury-black/70 hover:text-luxury-black' : 'text-white/70 hover:text-white'
                      }`}>
                        Admin
                      </Link>
                    )}
                    <Link to="/dashboard" className={`font-body text-xs uppercase tracking-[0.2em] font-medium transition-colors ${
                      scrolled ? 'text-luxury-black/70 hover:text-luxury-black' : 'text-white/70 hover:text-white'
                    }`}>
                      Dashboard
                    </Link>
                ...
                >
                  <Link
                    ...
                    className={`block font-display text-3xl py-4 transition-colors ${
                      isActive(link.to) ? 'text-luxury-accent' : 'text-luxury-black/60 hover:text-luxury-black'
                    }`}
                  >
                    {link.label}
                  </Link>
              ...
              >
                {user ? (
                  <div className="flex flex-col gap-3">
                    {user.role === 'admin' && <Link to="/admin" onClick={() => setMobileOpen(false)} className="font-body text-sm uppercase tracking-widest text-luxury-black/60 py-2">Admin</Link>}
                    <Link to="/dashboard" onClick={() => setMobileOpen(false)} className="font-body text-sm uppercase tracking-widest text-luxury-black/60 py-2">Dashboard</Link>
                    <button onClick={() => { logout(); setMobileOpen(false); }} className="btn-primary w-full text-center"><span>Sign Out</span></button>
                  </div>
                ) : (
                  <Link to="/login" onClick={() => setMobileOpen(false)} className="btn-primary w-full text-center"><span>Sign In</span></Link>
                )}
              </motion.div>
            </div>
          </motion.div>
